...
# Create the combined residuals plot:
combined_resid_plot <- ggplot(resid_combined, aes(x = Predicted, y = Residuals)) +
  geom_point(size = 1.5) +
...
# Create the combined Q-Q plot:
combined_qq_plot <- ggplot(resid_std_combined, aes(sample = StdResid)) +
  stat_qq() +
  stat_qq_line() +
  facet_wrap(~Model, ncol = 2, scales = "free") +
  ggtitle("Q-Q Plot") +
...
grid.arrange(combined_resid_plot, combined_qq_plot, nrow = 2)
```
...
threshold <- 5
large_deviation_indices_qq <- which(abs(resid_std.nb$StdResid) > threshold)
