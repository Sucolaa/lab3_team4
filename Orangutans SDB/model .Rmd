...
```{r}
complex.fit1 <- glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N` * `#AdoFemPresent`+ (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```

```{r}
complex.fit2 <- glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N`* `#AdultFemPreent`+ (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```

```{r}
complex.fit3 <- glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N`*`#UnflangedPresent` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```


```{r}
complex.fit4 <- glmer(SDB ~ `Age-Sex of Focal`* `Social Y or N`* `#FlangedPresent` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```
...
poi.fit <- glmer.nb(SDB ~ `Age-Sex of Focal` + `Social Y or N` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = AdolFem_Adul)
```
