...
```{r}
complex.fit1 <- stan_glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N` * `#`+ (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```

```{r}
complex.fit2 <- stan_glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```

```{r}
complex.fit3 <- stan_glmer(SDB ~ `Age-Sex of Focal`*`Social Y or N` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```


```{r}
complex.fit4 <- stan_glmer(SDB ~ age * sex + `Social Y or N` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = mix_sdb,family = poisson(link = "log"))
```
...
AdolFem_Adul <- mix_sdb %>% filter(`Age-Sex of Focal` == "Adol Female" & `#AdultPresent`>0) 
AdolFem_Adul <- AdolFem_Adul %>% mutate( `Age-Sex of Focal` = factor(AdolFem_Adul$`Age-Sex of Focal`),  `Social Y or N` = factor(AdolFem_Adul$`Social Y or N`, levels = c("Y", "N")),
         `Focal Orangutan ID` = factor(AdolFem_Adul $`Focal Orangutan ID`),
         `Event ID` = factor(AdolFem_Adul$`Event ID`))

buffer.nb <- glmer.nb(SDB ~ `Age-Sex of Focal` + `Social Y or N` + (1 | `Focal Orangutan ID`) + (1 | `Event ID`), data = AdolFem_Adul)
```
