...
# plot residuals
ggplot(resid_combined, aes(x = Predicted, y = Residuals, color = Model)) +
  geom_point(size = 1.5) +
...
poi.root <- ppc_rootogram(mix_sdb$SDB, posterior_predict(poi.fit), style = "hanging") +
  ggtitle("Poisson GLMM") + scale_fill_manual(values = "red") +
  coord_cartesian(xlim = c(0, 150))

nb.root <- ppc_rootogram(mix_sdb$SDB, posterior_predict(nb.fit), style = "hanging") +
  ggtitle("Negative Binomial GLMM") + scale_fill_manual(values = "red") +
  coord_cartesian(xlim = c(0, 150))
...
```{r,eval=FALSE,fig.align='center',fig.width=9}
# Create rootogram plots
poi.root <- ppc_rootogram(mix_sdb$SDB, posterior_predict(poi.fit), style = "hanging") +
